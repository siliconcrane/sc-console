...
```json
{
  ...
  "rules": [
    ...
    { "metric": "cvr_bp", "operator": "lt", "threshold": 100, "label": "Conversion rate below 1%" },
    {
      ...
      "operator": "gte",
      "threshold": 100000,
      "label": "Total spend reaches $1,000 budget cap"
    }
  ]
}
```
...
const result = await env.DB.prepare('SELECT * FROM experiments WHERE id = ?')
  .bind(experimentId)
  .first()

// ✅ CORRECT: Multiple parameters
const result = await env.DB.prepare('SELECT * FROM leads WHERE experiment_id = ? AND status = ?')
...
const result = await env.DB.prepare(`SELECT * FROM experiments ORDER BY ${sortField}`).first()
